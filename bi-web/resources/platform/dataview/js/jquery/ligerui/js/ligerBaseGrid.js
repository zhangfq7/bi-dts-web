...
(function ($)
{
    ...
    $.ligerDefaults.GridString = {
        ...
        pageStatMessage: '{from}-{to} &nbsp;&nbsp;共 {total} 条 ',
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        _render: function ()
        {
            ...
            gridhtmlarr.push("                    <div class='l-bar-button  l-bar-btnfirst '><span class='fa fa-step-backward'></span></div>");
            gridhtmlarr.push("                    <div class='l-bar-button l-bar-btnprev '><span class='fa fa-caret-left' style='font-size:17px'></span></div>");
            gridhtmlarr.push("                </div>");
            ...
            gridhtmlarr.push("                <div class='l-bar-group'>");
            gridhtmlarr.push("                     <div class='l-bar-button l-bar-btnnext'><span class='fa fa-caret-right' style='font-size:17px'></span></div>");
            gridhtmlarr.push("                    <div class='l-bar-button l-bar-btnlast'><span class='fa fa-step-forward'></span></div>");
            gridhtmlarr.push("                </div>");
        ...
        //zhump增加回调函数
        loadServerData: function (param, clause, sourceType,callback)
        {
            var g = this, p = this.options;
            var url = p.url;
        ...
        */
        ...
        _createHeaderCell: function (column)
        {
            ...
            if (column.ischeckbox)
            {
                ...
                jcell.html("<div class='l-grid-hd-cell-inner' style='margin-top:4px'><div class='l-grid-hd-cell-text l-grid-hd-cell-btn-checkbox'></div></div>");
            }
            if (column.isdetail)
            {
	    	...
	    	//zhump:用户设置行高,需要上下居中
                jcell.css({
                	height:p.headerRowHeight,
                	"line-height":p.headerRowHeight+'px'
                });
            }
            if (column['__leaf'])
            {
                ...
                $(g.columns).each(function (columnindex, column)
                {
                    ...
                    if (this.ischeckbox)
                    {
                        ...
                        if (p.fixedCellHeight)
                            ...
                            gridhtmlarr.push(' style = "min-height:' + p.rowHeight + 'px;line-height:' + p.rowHeight + 'px" ');
                        gridhtmlarr.push('>');
                        gridhtmlarr.push('<span class="l-grid-row-cell-btn-checkbox" style="margin-top:7px;"></span>');
        ...
        _getTreeCellHtml: function (oldContent, rowdata)
        {
            ...
            if (isExtend && isParent) {
				...
				if(rowdata.iconClass){
					content += "<span class='bi-attr-icon "+rowdata.iconClass+"'></span>";
				}
				content += "<span class='l-grid-tree-content' file>" + oldContent + "</span>";
			}
            return content;
        },
