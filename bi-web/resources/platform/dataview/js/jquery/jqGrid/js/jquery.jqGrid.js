...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
				...
				case "script":
					$.ajax($.extend({
						...
						success:function(data,st, xhr) {
							...
							if(data){
								var len = $.isArray(data)? data.length:data.rows.length;
								if(len== 0){
									noMsg
									var noMsg =ts.p.noMsg || $.jgrid.getRegional(ts, "defaults.noresult", ts.p.noresult );
									$(ts).parents(".ui-jqgrid-bdiv").append('<div class="noResult" style="font-size:12px;height:28px;line-height:35px;text-align:center;">'+noMsg+'</div>');
									$(ts).parents(".ui-jqgrid-bdiv").find(".noResult").prev().hide();
								}else{
									var $noResult = $(ts).parents(".ui-jqgrid-bdiv").find(".noResult");
									$noResult.prev().show();
									$noResult.remove();
								}
							}
		...
		$("th",thr).each(function ( j ) {
			...
			$(this).css("text-align",ts.p.colModel[j].hlign||'left');
			res = null;
			var hdcol = "";
