...
(function($) {
	...
	$.Poshytip.prototype = {
		...
		show: function() {
			...
			var _this = this;
			if(this.opts.showBefore){
				this.opts.showBefore.call(this.$elm,function(){
					show(_this)
				});
			}else{
				show(_this)
			...
			function show(_this){
				...
				_this.display();
				if (_this.opts.timeOnScreen)
					_this.hideDelayed(_this.opts.timeOnScreen);
				if(_this.opts.show){
					_this.opts.show.call(_this.$elm);
				}
			}
		...
		hide: function() {
			...
			if(this.opts.hideBefore){
				this.opts.hideBefore.call(this.$elm);
			}	
			this.display(true);
			if(this.opts.hide){
				this.opts.hide.call(this.$elm);
			}
		},
		...
		display: function(hide) {
			...
			hide ? this.$tip.queue($.proxy(this.reset, this)) : this.$tip.css('visibility', 'visible');
			if (active) {
				var title = this.$elm.data('title.poshytip');
				if (title !== null)
					this.$elm.attr('title', title);
